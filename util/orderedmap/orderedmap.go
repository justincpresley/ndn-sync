...
func (om *OrderedMap[K, V]) Set(key K, value V, old bool) (val V, present bool) {
	if pair, present := om.pairs[key]; present {
		oldValue := pair.Value
		pair.Value = value
		if !old {
	...
	om.setNewPair(pair, old)
	return
}

// Store is an alias for Set, mostly to present an API similar to `sync.Map`'s.
func (om *OrderedMap[K, V]) Store(key K, value V, old bool) (V, bool) {
	return om.Set(key, value, old)
}
...
func (om *OrderedMap[K, V]) setNewPair(pair *Pair[K, V], old bool) {
	if old {
		pair.element = om.list.PushBack(pair)
	} else {
		pair.element = om.list.PushFront(pair)
	}
