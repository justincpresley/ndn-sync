...
func TestBasicFeatures(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		oldValue, present := om.Set(i, 2*i, true)
	...
	for j := 0; j < n/2; j++ {
		i = 2 * j
		oldValue, present := om.Set(i, 4*i, true)
...
func TestUpdatingDoesntChangePairsOrder(t *testing.T) {
	...
	oldValue, present := om.Set("po", 102, true)
...
func TestDeletingAndReinsertingChangesPairsOrder(t *testing.T) {
	...
	oldValue, present = om.Set("po", 100, true)
...
func TestPackUnpackStructs(t *testing.T) {
	...
	om.Set("foo", dummyTestStruct{"foo!"}, true)
	om.Set("bar", dummyTestStruct{"bar!"}, true)
	...
	value, present = om.Set("bar", dummyTestStruct{"baz!"}, true)
	assert.True(t, present)
	if assert.NotNil(t, value) {
	...
	for _, n := range []int{0, 10, 20, 100, 1000, 10000} {
		t.Run(fmt.Sprintf("shuffle test with %d items", n), func(t *testing.T) {
			...
			for i := 0; i < n; i++ {
				...
				value, present := om.Set(keys[i], values[i], true)
